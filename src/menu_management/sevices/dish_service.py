...
class DishService:
    ...
    async def get_all_dishes(self, submenu_id: str) -> list[Dish]:
        ...
        await self.redis_cache.set_cache('all_dish', submenu_id, dishes)
        return dishes
    ...
    async def get_dish(self, dish_id: str) -> Dish:
        ...
        if dish:
            await self.redis_cache.set_cache('dish', dish_id, dish)
            return dish
    ...
    async def post_dish(self, submenu_id: str, menu_id: str, dish: CreateDish) -> Dish:
        ...
        await self.redis_cache.set_cache('dish', submenu_id, new_dish)
    ...
    async def patch_dish(self, submenu_id: str, dish: PatchDish) -> Dish:
        ...
        if patched_dish:
            await self.redis_cache.clear_cache('all_dish', submenu_id)
            await self.redis_cache.set_cache('dish', patched_dish.id, patched_dish)
            return patched_dish
        raise HTTPException(status_code=404, detail='dish not found')
    ...
    async def delete(self, dish_id: str, submenu_id: str) -> dict[str, bool | str]:
        ...
        await self.redis_cache.clear_cache('submenu', 'all_submenu')
        await self.redis_cache.clear_cache('menu', 'all_menu')
        await self.redis_cache.clear_cache('dish', dish_id)
        return result
