...
async def get_submenus(menu_id: str):
    return await SubmenuService.get_all_submenus(menu_id)
...
async def get_specific_submenu(submenu_id: str):
    return await SubmenuService.get_submenu(submenu_id)
...
async def add_submenu(menu_id: str, new_submenu: CreateSubmenu):
    return await SubmenuService.post_submenu(menu_id, new_submenu)
...
@submenu_router.delete('/{submenu_id}', status_code=200,
...
async def delete_menu(submenu_id: str, menu_id: str):
    return await SubmenuService.delete(submenu_id, menu_id)
